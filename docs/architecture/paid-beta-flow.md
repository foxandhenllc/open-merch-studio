...
- `/api/admin/orders` lists order summaries from runtime state or the database fallback.
...
Fixture mode uses an in-memory runtime store. When `DATABASE_URL` is configured, the Prisma path persists sessions, AI spend events, design assets, mockup task outcomes, quotes, orders, transitions, payment events, fulfillment attempts, admin settings, and audit logs. Production persistence still needs migration/runbook verification against the dedicated paid-beta database.
