// Generated by npm run config:generate. Edit config/merchant.config.json instead.

export const merchantConfig = {
  $schema: './merchant.schema.json',
  schemaVersion: 1,
  brand: {
    displayName: 'Open Merch Studio',
    shortName: 'OMS',
    shortDescription: 'Open-source custom merch workbench',
    logoPath: '/icon.svg',
    socialImagePath: '/open-merch-studio-share.png',
    colors: {
      background: '#f5f5f2',
      foreground: '#171814',
      accent: '#3154d9',
    },
  },
  web: {
    canonicalUrl: 'https://openmerchstudio.com',
    title: 'Open Merch Studio | Create Original Custom Merch',
    description:
      'Upload artwork, create from references, improve photos for print, preview placements, and order custom merchandise in Open Merch Studio.',
  },
  operator: {
    legalName: 'Fox&Hen, LLC',
    disclosure: 'Open Merch Studio is operated by Fox&Hen, LLC.',
    supportEmail: 'support@openmerchstudio.com',
    countryCode: 'US',
  },
  orders: {
    prefix: 'OMS',
  },
  email: {
    senderName: 'Open Merch Studio',
    fromLocalPart: 'orders',
  },
  catalog: {
    currency: 'USD',
    shippingCountryCodes: ['US'],
    curatedCategorySlugs: ['apparel', 'drinkware', 'wall-art', 'bags', 'stickers'],
  },
  pricing: {
    marginLabel: 'Open Merch Studio margin',
  },
  policies: {
    privacyPath: '/privacy',
    termsPath: '/terms',
    returnsPath: '/returns',
    contentPolicyPath: '/content-policy',
    approvedVersion: '2026-09-03',
    lastApprovedDate: '2026-09-03',
    contentFile: 'policies/open-merch-studio.json',
    contentSha256: '39e9a66c6bc474cfeb2735829dffff1de7fd738728054f4e9eb52c16191d90fa',
  },
  attribution: {
    projectName: 'Open Merch Studio',
    projectUrl: 'https://openmerchstudio.com',
    sourceUrl: 'https://github.com/foxandhenllc/open-merch-studio',
    licenseName: 'MIT',
    creatorName: 'Fox & Hen',
    creatorUrl: 'https://foxandhenllc.com/merch',
  },
} as const;

export type MerchantConfig = typeof merchantConfig;
